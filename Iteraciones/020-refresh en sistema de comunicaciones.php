...
function smtp_read($fp){
  ...
  while (!feof($fp)) {
    ...
    if ($line === false) break;
    $data .= $line;
    // Cuando aparece "XYZ " termina la respuesta
 ...
 * No usa librerías. Hace el flujo SMTP “a mano”.
 *
 * @return bool true si envió OK; false si falló, con $err descriptivo.
 */
function smtp_send_gmail($host, $port, $user, $pass, $fromEmail, $fromName, $to, $subject, $body, &$err){
  ...
  // Dot-stuffing: si una línea comienza con ".", se duplica.
  $lines = preg_split("/\r\n|\n|\r/", (string)$body);
  $safeBody = "";
  foreach($lines as $ln){
...
/* ---------------- IMAP: obtener body en texto plano ---------------- */

/**
 * Devuelve el cuerpo del mensaje priorizando text/plain.
 * - Si es multipart, busca la parte text/plain.
 * - Si no, cae a imap_body().
 */
...
/* =========================================================
   ...
   - Redirige al informe y baja a comunicaciones (refresh)
========================================================= */
if ($loggedIn && isset($_POST["action"]) && $_POST["action"] === "send_email") {
  ...
  if ($ok) {
    // Guardamos el envío en BD como historial interno del CRM
...
if ($loggedIn && $emailCol !== null && function_exists('imap_open')) {
  ...
  if ($imap) {
    ...
    // limpia warnings acumulados (reduce notices al final)
...
/* =========================================================
   VISTA INFORME + comunicaciones (IMAP)
========================================================= */
...
if ($loggedIn && $viewId !== null && $viewId !== "") {
  ...
  if (!$emailCol) {
    ...
    if (!$imap) {
      ...
      /**
       * Busca en un mailbox (INBOX o All Mail) usando:
       * - FROM "email"
       * - TO "email"
       * - fallback: TEXT "email"
       */
...
/* =========================================================
   Si llegó refresh=1, opcionalmente puedes mostrar flash:
   (No molesta, solo informa)
========================================================= */
if ($loggedIn && isset($_GET["refresh"]) && $_GET["refresh"] == "1") {
  // Si ya hay un mensaje, no lo pisa.
        ...
        <div class="grid-2">
          ...
          <div class="card" id="comms">
            <div style="display:flex; justify-content:space-between; gap:10px; align-items:center; flex-wrap:wrap;">
              ...
              <!-- BOTÓN REFRESH (recarga SOLO el informe y te deja aquí) -->
