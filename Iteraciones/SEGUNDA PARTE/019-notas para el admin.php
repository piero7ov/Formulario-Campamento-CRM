 ...
 *      - Create (INSERT)
 *      - Edit (UPDATE)
 *      - Delete (DELETE + confirmación)
 *  - Gestión de datos (v1): Notas internas por inscripción
 *
 * Requisitos:
 *  - PHP con mysqli
...
/* =========================================================
   ...
   - page=ops   : operaciones (CRUD, datos, mantenimiento)
========================================================= */
...
/* =========================================================
   ...
   - crm_notas: notas internas por inscripción (v1)
========================================================= */
$c->query("
...
/* =========================================================
   16.5) ✅ NOTAS (Gestión de datos v1)
   ---------------------------------------------------------
   - note_add: crear nota interna
   - note_delete: eliminar nota
========================================================= */
if ($loggedIn && isset($_POST["action"]) && $_POST["action"] === "note_add") {
  $id_registro = trim((string)($_POST["id_registro"] ?? ""));
  $nota        = trim((string)($_POST["nota"] ?? ""));
  $return      = trim((string)($_POST["return"] ?? ""));

  // Return seguro (misma lógica simple: relativo)
  $safeReturn = $_SERVER["PHP_SELF"]."?page=ops&tab=datos";
  if ($return !== "") {
    $u = parse_url($return);
  ...
  if ($id_registro === "" || $nota === "") {
    flash_set("flash_err", "Completa el ID de inscripción y la nota.");
    header("Location: " . $safeReturn);
    exit;
  }

  // Verificar que exista la inscripción (si tenemos PK)
  if ($primaryKeyColumn) {
    $stmt = $c->prepare("SELECT 1 FROM `$table_name` WHERE `$primaryKeyColumn` = ? LIMIT 1");
    if ($stmt) {
      ...
      if (!$exists) {
        flash_set("flash_err", "No existe una inscripción con ID #".$id_registro.".");
        header("Location: " . $safeReturn);
        exit;
      }
    }
  ...
  $stmt = $c->prepare("INSERT INTO crm_notas (id_registro, nota, created_by) VALUES (?, ?, ?)");
  if (!$stmt) {
    flash_set("flash_err", "No se pudo guardar la nota (prepare).");
    header("Location: " . $safeReturn);
    exit;
  }

  $createdBy = $ADMIN_USER;
  $stmt->bind_param("sss", $id_registro, $nota, $createdBy);
  $ok = $stmt->execute();
  $stmt->close();

  if ($ok) {
    flash_set("flash_ok", "Nota guardada en la inscripción #".$id_registro.".");
  } else {
    flash_set("flash_err", "No se pudo guardar la nota (execute).");
  }

  // Si venías del informe, vuelve al ancla de notas
  header("Location: " . $safeReturn);
  exit;
}

if ($loggedIn && isset($_POST["action"]) && $_POST["action"] === "note_delete") {
  ...
  if ($note_id <= 0) {
    flash_set("flash_err", "Falta el ID de la nota a eliminar.");
    header("Location: " . $safeReturn);
    exit;
  }

  $stmt = $c->prepare("DELETE FROM crm_notas WHERE id = ? LIMIT 1");
  if ($stmt) {
    $stmt->bind_param("i", $note_id);
    $ok = $stmt->execute();
    $stmt->close();

    if ($ok) flash_set("flash_ok", "Nota eliminada.");
    else     flash_set("flash_err", "No se pudo eliminar la nota.");
  } else {
    flash_set("flash_err", "No se pudo eliminar la nota (prepare).");
  }
...
/* =========================================================
   20) VISTA INFORME + comunicaciones + notas
========================================================= */
...
if ($loggedIn && $viewId !== null && $viewId !== "") {
  ...
  // cargar notas
  $stmt = $c->prepare("
    SELECT id, nota, created_by, created_at
    FROM crm_notas
    WHERE id_registro = ?
...
if ($loggedIn && isset($_POST["action"]) && $_POST["action"] === "crud_delete") {
  ...
  /**
   * Return seguro:
   * - Desde el listado mandamos $_SERVER["REQUEST_URI"] para volver con filtros/orden
   * - Pero lo validamos: solo rutas relativas y a este mismo archivo
   */
  $return = trim((string)($_POST["return"] ?? ""));
  ...
  if ($return !== "") {
    ...
    if (is_array($u) && !isset($u["scheme"]) && !isset($u["host"])) {
      $path = (string)($u["path"] ?? "");
      if ($path === "" || basename($path) === basename($_SERVER["PHP_SELF"])) {
        $safeReturn = $return; // OK, es local al mismo script
      }
    }
  }
  ...
  try {
    ...
    // 0) Borrar notas (tabla auxiliar)
    $stmt = $c->prepare("DELETE FROM crm_notas WHERE id_registro = ?");
    if (!$stmt) { $ok = false; }
    else {
      $stmt->bind_param("s", $id);
      $ok = $stmt->execute();
...
<head>
  ...
  <link rel="stylesheet" href="admin_019.css">
    ...
    <?php if ($viewId !== null && $viewId !== ""): ?>
      ...
      <?php else: ?>
        ...
        <div class="card" id="notes">
          <div class="row-between">
            <div class="section-title">Notas internas (<?= count($notesByView) ?>)</div>
            <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>?page=ops&tab=datos">Abrir Gestión de datos</a>
          </div>
          ...
          <form method="post" class="mt-12">
            ...
            <input type="hidden" name="return" value="<?= h($_SERVER["PHP_SELF"].'?view='.urlencode($viewId).'#notes') ?>">

            <div class="control">
              <label>Nueva nota</label>
              <textarea name="nota" placeholder="Escribe una nota interna (solo admin)..." required></textarea>
              <div class="help">Se guarda como registro interno del panel.</div>
            </div>

            <div class="form-actions">
              <button class="btn-estado" type="submit">Guardar nota</button>
            </div>
          </form>

          <div class="notes-list mt-12">
            <?php if (!$notesByView): ?>
              <div class="tiny">— Sin notas todavía —</div>
            <?php else: ?>
              <?php foreach($notesByView as $n): ?>
                <div class="note-item">
                  <div class="note-meta">
                    <span><strong>#<?= h($n["id"]) ?></strong> · <?= h($n["created_at"]) ?></span>
                    <span><?= h($n["created_by"] ?: "admin") ?></span>
                  </div>
                  <div class="note-body"><?= h($n["nota"]) ?></div>

                  <div class="note-actions">
                    <form method="post" class="js-delete-form" style="display:inline;"
                          data-confirm="¿Eliminar la nota #<?= h($n["id"]) ?> de la inscripción #<?= h($viewId) ?>?">
                      <input type="hidden" name="action" value="note_delete">
                      <input type="hidden" name="note_id" value="<?= h($n["id"]) ?>">
                      <input type="hidden" name="return" value="<?= h($_SERVER["PHP_SELF"].'?view='.urlencode($viewId).'#notes') ?>">
                      <button type="submit" class="btn-link btn-danger">Eliminar nota</button>
                    </form>
                  </div>
                </div>
              <?php endforeach; ?>
    ...
    <?php else: ?>
      ...
      <?php if ($page === "ops"): ?>
        ...
        <?php if ($opsTab === "datos"): ?>
          ...
          <?php
            // Filtros simples para listado de notas
            $nf_id = trim((string)($_GET["nid"] ?? ""));
            $nf_q  = trim((string)($_GET["q"] ?? ""));
            ...
            $sql = "SELECT id, id_registro, nota, created_by, created_at
                    FROM crm_notas
          ...
          <div class="card">
            <div class="row-between">
              <div class="section-title">📝 Notas internas por inscripción</div>
            </div>

            <form method="post" class="mt-12">
              <input type="hidden" name="action" value="note_add">
              <input type="hidden" name="return" value="<?= h($_SERVER["PHP_SELF"].'?page=ops&tab=datos') ?>">

              <div class="grid-form">
                <div class="control">
                  <label>ID de inscripción</label>
                  <input class="input" type="text" name="id_registro" placeholder="Ej: 15" required>
                  <div class="help">Debe existir en la tabla principal.</div>
                </div>

                <div class="control">
                  <label>Nota</label>
                  <textarea name="nota" placeholder="Escribe la nota interna..." required></textarea>
                </div>
              </div>
          ...
          <div class="card">
            <div class="row-between">
              <div class="section-title">📌 Últimas notas (<?= count($notesList) ?>)</div>
              <span class="tiny">Máx. 200 resultados</span>
            </div>
            ...
            <form method="get" class="mt-12">
              ...
              <div class="grid-form">
                <div class="control">
                  ...
                  <input class="input" type="text" name="nid" value="<?= h($nf_id) ?>" placeholder="Ej: 15">
                </div>
                <div class="control">
                  <label>Buscar texto</label>
                  <input class="input" type="text" name="q" value="<?= h($nf_q) ?>" placeholder="Ej: pago, documento, llamar...">
                </div>
              </div>

              <div class="form-actions">
                <button class="btn-estado" type="submit">Aplicar</button>
            ...
            <div class="notes-list mt-12">
              <?php if (!$notesList): ?>
                <div class="tiny">— Sin notas con esos filtros —</div>
              <?php else: ?>
                <?php foreach($notesList as $n): ?>
                  <div class="note-item">
                    <div class="note-meta">
                      <span>
                        <strong>#<?= h($n["id"]) ?></strong>
                        · <?= h($n["created_at"]) ?>
                        · Inscripción: <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>?view=<?= h($n["id_registro"]) ?>#notes">#<?= h($n["id_registro"]) ?></a>
                      </span>
                      <span><?= h($n["created_by"] ?: "admin") ?></span>
                    </div>
                    ...
                    <div class="note-actions">
                      <form method="post" class="js-delete-form" style="display:inline;"
                            data-confirm="¿Eliminar la nota #<?= h($n["id"]) ?> (Inscripción #<?= h($n["id_registro"]) ?>)?">
                        <input type="hidden" name="action" value="note_delete">
                        <input type="hidden" name="note_id" value="<?= h($n["id"]) ?>">
                        <input type="hidden" name="return" value="<?= h($_SERVER["PHP_SELF"].'?page=ops&tab=datos&do=create&nid='.urlencode($nf_id).'&q='.urlencode($nf_q)) ?>">
                        <button type="submit" class="btn-link btn-danger">Eliminar</button>
                      </form>

                      <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>?view=<?= h($n["id_registro"]) ?>#notes">Ver en informe</a>
                    </div>
                  </div>
                <?php endforeach; ?>
              <?php endif; ?>
        ...
        <?php elseif ($opsTab === "crud" && $opsAction === "create"): ?>
          <div class="card">
            <div class="row-between">
          ...
          <div class="card">
            ...
            <?php else: ?>
              ...
              <div class="row-actions mt-12">
                <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>?view=<?= h($editId) ?>#notes">Ver informe</a>
                <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>">Volver al listado</a>
              ...
              <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-actions">
                  ...
                  <a class="btn-link" href="<?= h($_SERVER["PHP_SELF"]) ?>?view=<?= h($editId) ?>#notes">Cancelar</a>
                </div>
              </form>
