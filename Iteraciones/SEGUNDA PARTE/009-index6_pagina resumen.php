...
<form id="frmCampamento" action="" method="POST" enctype="multipart/form-data">
    <fieldset>
        ...
        <!-- Limpiar arriba a la derecha (misma estética, sin emojis y discreto) -->
        ...
        while($f = $r->fetch_assoc()){
            ...
            // Comentario SQL (SIEMPRE: obligatorios + opcionales)
            echo $comentario;
...
(function(){
  ...
  // Cargar draft (solo si NO estamos limpiando)
  if (!mustClearDraft) loadDraft();

  // Guardar draft al escribir/cambiar
  ...
  if (btnClear) {
    btnClear.addEventListener("click", function(e){
      ...
      // Limpia también lo pendiente del servidor y recarga limpio
  ...
  function buildSummary(){
    ...
    const els = form.querySelectorAll("input, select, textarea");
    els.forEach(el => {
      // No mostrar archivos (blobs) en el resumen
      if (el.type === "file") return;

      // Ignora submit/buttons
      if (el.type === "submit" || el.type === "button") return;

      // Ignora elementos sin name (por si acaso)
      if (!el.name) return;
  ...
  // Interceptar submit para mostrar resumen
  form.addEventListener("submit", function(e){
    // Si el navegador detecta required/email/...
    if (!form.checkValidity()){
      // Muestra mensajes nativos y no abre modal
      form.reportValidity();
      return;
    }

    // Abrimos modal y evitamos enviar todavía
    e.preventDefault();
    buildSummary();
    openModal();
  });
  ...
  btnConfirmar.addEventListener("click", function(){
    // Guardar por última vez (por si acaso) y enviar
    saveDraft();
    closeModal();
    form.submit(); // envía de verdad (mantiene archivos)
  });
