...
/* =========================================================
   ...
   ---------------------------------------------------------
   - Se ejecuta SOLO por POST
   - Confirmación se hace en JS (front) con window.confirm()
   - Aquí: borra registro principal + limpia tablas auxiliares
...
if ($loggedIn && isset($_POST["action"]) && $_POST["action"] === "crud_delete") {
  ...
  try {
    ...
    // Cualquier excepción => rollback + mensaje
    @($c->rollback());
...
<head>
  ...
  <link rel="stylesheet" href="admin_017.css">
