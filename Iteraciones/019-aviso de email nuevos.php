...
/* =========================================================
   HELPERS (con comentarios explicativos)
   ========================================================= */

/**
 * Escape seguro para HTML (evita XSS al imprimir en pantalla).
 * @param mixed $s
 * @return string
 */
...
/**
 * Intenta detectar la columna de email/correo en una lista de columnas.
 * Busca por nombres que contengan "email", "mail" o "correo".
 * @param array $cols
 * @return string|null
 */
...
/**
 * Decodifica asuntos/remitentes con codificación MIME (ej: =?UTF-8?B?...?=).
 * Esto evita que veas caracteres raros en "Subject" o "From".
 * @param string $str
 * @return string
 */
...
/**
 * Extrae emails desde el header IMAP (header->from) y los normaliza a minúsculas.
 * Útil para construir el mapa de "emails no leídos" por contacto.
 * @param object $header
 * @return array (emails únicos)
 */
...
/**
 * Lee la respuesta del servidor SMTP (maneja respuestas multi-línea).
 * @param resource $fp
 * @return string
 */
function smtp_read($fp){
  ...
  while (!feof($fp)) {
    ...
    if ($line === false) break;
    $data .= $line;
    // Cuando aparece "XYZ " (código + espacio) termina respuesta
...
/**
 * Envía un comando SMTP y valida el código esperado.
 * @param resource $fp
 * @param string|null $cmd
 * @param int|null $expectCode
 * @return array [bool ok, string resp]
 */
...
/**
 * Codifica un texto para cabecera MIME si hay caracteres especiales.
 * @param string $text
 * @return string
 */
...
/**
 * Envía un email por SMTP usando STARTTLS (sin librerías externas).
 * Implementa AUTH LOGIN + MAIL FROM + RCPT TO + DATA.
 * @return bool true si envió, false si falló (y deja $err con detalle)
 */
function smtp_send_gmail($host, $port, $user, $pass, $fromEmail, $fromName, $to, $subject, $body, &$err){
  ...
  // Dot-stuffing (si una línea empieza con punto, se duplica)
  $lines = preg_split("/\r\n|\n|\r/", (string)$body);
  $safeBody = "";
  foreach($lines as $ln){
...
/**
 * Obtiene el cuerpo del correo priorizando text/plain si el mensaje es multipart.
 * - Primero busca un part text/plain.
 * - Si no existe, cae a imap_body().
 * @param resource $imap
 * @param int $msgno
 * @return string
 */
...
/* =========================================================
   COLUMNAS DE INSCRIPCIONES + detectar columna email
   ========================================================= */
$cols = [];
$emailCol = null;
if ($loggedIn) {
...
/* =========================================================
   (NUEVO) MAPA DE EMAILS NO LEÍDOS por contacto (para la tabla)
   - Abre IMAP una sola vez
   - Busca UNSEEN en INBOX
   - Cuenta cuántos hay por remitente
   ========================================================= */
$unreadCountByEmail = []; // ejemplo: ["cliente@mail.com" => 2]
if ($loggedIn && $emailCol !== null && function_exists('imap_open')) {
  $imap = @imap_open($hostname, $username, $password);

  if ($imap) {
    // Mensajes no leídos en INBOX
    ...
    if (is_array($unseenIds)) {
      foreach ($unseenIds as $num) {
        ...
        foreach ($fromEmails as $fromEmail) {
          // ignoramos nuestros propios correos
          if ($fromEmail === strtolower($username)) continue;

          if (!isset($unreadCountByEmail[$fromEmail])) $unreadCountByEmail[$fromEmail] = 0;
          $unreadCountByEmail[$fromEmail] += 1;
        }
      }
    }

    // Limpia alertas/errores IMAP acumulados (evita warnings en shutdown)
...
if ($loggedIn && $viewId !== null && $viewId !== "") {
  ...
  if (!$emailCol) {
    ...
    if (!$imap) {
      ...
      // Busca IDs en una carpeta (mailbox) combinando FROM y TO (sin OR)
      ...
      // 2) Si no hay nada, intenta All Mail / Todos
      if (empty($ids)) {
...
<head>
  ...
  <style>
    *{ box-sizing:border-box; }
    ...
    th.col-unread, td.col-unread{ min-width: 110px; text-align: center; }
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="col-unread" title="Correos no leídos (UNSEEN) en INBOX">Emails nuevos</th>
          ...
          <tbody>
            <?php
              ...
              while($r && ($f = $r->fetch_assoc())) {
                ...
                // email normalizado del registro (si existe)
                $rowEmailNorm = null;
